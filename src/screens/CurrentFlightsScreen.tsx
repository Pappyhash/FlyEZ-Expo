...
import { getCurrentUserTrips } from '../graphql/queries';

const client = generateClient();

const CurrentFlightsScreen = ({route, navigation}) => {
  ...
  const isFocused = useIsFocused();
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            if (userId) {
                try {
                    const queryResponse = await client.graphql({ query: getCurrentUserTrips, variables: { user: userId }});
                    const tripList: TripsModel[] = [];
        
                    if (queryResponse.data.getCurrentUserTrips) {
                        queryResponse.data.getCurrentUserTrips.forEach(trip => {
                            if (trip) { 
                                tripList.push(trip);
                            }
                        });
...
export default CurrentFlightsScreen;